...
HighLevelConsumer.prototype.connect = function () {
    ...
    this.on('rebalanced', function () {
        if (self.options.fromOffset) {
            self.ready = true;
            self.setOffsetsToInitialPayload();
            self.fetch();
        } else {
...
HighLevelConsumer.prototype.setOffsetsToInitialPayload = function () {
    var clientToPartitionToOffset = {};
    // build mapping of client -> partition -> offset from initial payload
    _.each(this.payloads, function(payload) {
        if (!_.has(clientToPartitionToOffset, payload.topic)) {
            clientToPartitionToOffset[payload.topic] = {};
        }
        clientToPartitionToOffset[payload.topic][payload.partition] = payload.offset;
    });
    // set current payload offsets based on initial payload
    this.topicPayloads.map(function(topicPayload) {
        if (_.has(clientToPartitionToOffset, topicPayload.topic, topicPayload.partition)) {
            topicPayload.offset = clientToPartitionToOffset[topicPayload.topic][topicPayload.partition];
        }
    });
};
...
HighLevelConsumer.prototype.onDone = function (topics) {
    var self = this;
